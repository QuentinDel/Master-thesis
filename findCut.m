...
for i = -1 : 1
  if periodImplied + i >= 1 && periodImplied + i <= size(periodIdNotTransmit, 1)
     ...
     for j = 1 : length(idImplied)
       [nb, dist] = checkWhereElseIsImpliedId(idImplied(j), periodImplied + i, indiceCol + 1, positionsCol, periodSlot, intervTransmiss, periodImplied);
...
function [nb, dist] = checkWhereElseIsImpliedId(id, periodImplied, indiceCol, positionsCol, periodSlot, intervTransmiss, periodCollision)
nb = 0;
dist = [];
if indiceCol <= length(positionsCol)
    ...
    if periodImpliedNewCol == periodCollision+1 %Case where it is in the next detection period, need to cut here.
        return;
    end
    ...
    if intervTransmiss(id, 1) < pos && pos < intervTransmiss(id, 2)
       [nb, distBis] = checkWhereElseIsImpliedId(id, periodImplied, indiceCol + 1, positionsCol, periodSlot, intervTransmiss, periodCollision);
       nb = nb + 1;
       dist = pos;
       dist = [dist, distBis];
    end
